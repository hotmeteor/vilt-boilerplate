<template>
    <Combobox as="div" v-model="selectedOption">
        ...
        <div :class="['relative', label ? 'mt-1' : '']">
            <ComboboxInput class="w-full border-0 bg-white py-2 pl-3 pr-10 shadow-sm focus:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-500 sm:text-sm" @change="query = $event.target.value" :display-value="(option) => option?.label" />
                ...
                <ComboboxOption v-for="option in filteredOptions" :key="option.value" :value="option" as="template" v-slot="{ active, selected }">
                    <li :class="['relative cursor-default select-none py-2 pl-3 pr-9', active ? 'bg-indigo-600 text-white' : 'text-gray-900']">
                        <div class="flex items-center">
                            <span :class="['inline-block h-2 w-2 flex-shrink-0 rounded-full', option.color ? `bg-[${option.color}]` : 'bg-gray-200']" aria-hidden="true" />
